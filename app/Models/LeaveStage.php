...
class LeaveStage extends Model
{
    ...
    public function avilableStage($stageName)
    {
        ...
        foreach ($this->states as $state => $value) {
            if ($stageName != $state) {
                $original_leave_stages = LeaveStage::where('leave_id', $this->leave_id)->where('stage_name', $state)->where('status','!=','approved')->count();
    ...
    public function stageEmployees($stage_name)
    {
        return LeaveStage::where('stage_name', $stage_name)->where('leave_id', $this->leave_id)->pluck('responsible_employee')->toArray();
    }
